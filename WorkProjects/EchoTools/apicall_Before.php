				...
				if($validForm){
					...
					*/
					...
					Schedule {
						...
						durationMinutes (integer): Duration of each scheduled capture,
						sectionId (string, optional): ID of the Section in which to Schedule the Capture,
						name (string): Name of the Schedule,
						roomId (string): ID of the Room in which to Schedule the Capture,
						instructorId (string, optional): ID of the Instructor,
						guestInstructor (string, optional): Name of any Guest Instructor,
						shouldCaption (boolean): Boolean value indicating the Capture should be Captioned,
						shouldAutoPublish (boolean): Boolean value indicating the Capture should be Automatically Published,
					...
					*/
					...
					// Duration
					$postData['durationMinutes']				= ($_POST['inputDuration'] - 1);
					...
					// Room ID
					$postData['roomId']						= $_POST['inputRoom'];
					// Instructor ID
					$postData['instructorId']				= $_SESSION['user']['id'];
					// Caption video?
					$postData['shouldCaption']				= false;
					// Auto publish video?
					$postData['shouldAutoPublish']			= ($_POST['inputStreamLive'] == 1 || $_POST['inputAutoPublish'] == 1 ? true : false);
					// Stream video live?
					...
					// Duration (9-239)
					elseif(!preg_match('/(1[0-9][0-9]|2[0-3][0-9]|[1-9][0-9]|9)/',$postData['durationMinutes'])){ $validForm = false; $error = 315; }
					elseif($postData['durationMinutes'] < 9 || $postData['durationMinutes'] > 239){ $validForm = false; $error = 316; }
					// Section, room and instructor (all uuid)
					elseif(!preg_match('/([a-f]|[0-9]){8}-([a-f]|[0-9]){4}-([a-f]|[0-9]){4}-([a-f]|[0-9]){4}-([a-f]|[0-9]){12}/',$postData['sectionId'])){ $validForm = false; $error = 317; }
					elseif(!preg_match('/([a-f]|[0-9]){8}-([a-f]|[0-9]){4}-([a-f]|[0-9]){4}-([a-f]|[0-9]){4}-([a-f]|[0-9]){12}/',$postData['roomId'])){ $validForm = false; $error = 318; }
					elseif(!preg_match('/([a-f]|[0-9]){8}-([a-f]|[0-9]){4}-([a-f]|[0-9]){4}-([a-f]|[0-9]){4}-([a-f]|[0-9]){12}/',$postData['instructorId'])){ $validForm = false; $error = 319; }
					...
					if($validForm){
						...
						$response = apiCall('/public/api/v1/schedules','POST',json_encode($postData));

						// Schedule conflict
						if(strpos($response,"Overlapping")){
							$error = 320;
						}
						...
						else if(strpos($response,"Schedule Time in past")){
							$error = 323;
						}
						// Section not found
			...
			case 'deleteSchedule':
				...
				if($resultSQL){
					...
					if($affectedSQL == 1){
						...
						$response = apiCall('/public/api/v1/schedules/'.$id.'?access_token='.$_SESSION['auth']['accessToken'],'DELETE');
						$response = json_decode($response,true);
